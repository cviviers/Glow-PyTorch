...
def toTensor(sample):
    return torch.from_numpy(sample.copy()).type(torch.float32)
...
def GetImagesOnly(data):
    ...
    if isinstance(data, FilteredImageData):
        return np.stack([data.low_img, data.high_img], axis=0) 
    else: # isinstance(data, ImageData):

        return np.expand_dims(np.array(data.img), axis=0)
